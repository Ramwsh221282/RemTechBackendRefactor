...
import { GetSparesQueryResponse, SpareResponse } from '../../../../shared/api/spares-module/spares-api.responses';
...
@Component({
	...
	imports: [Select, FormsModule, FastNavigationVehiclesComponent, FastNavigationSparesComponent],
...
export class FastNavigationComponent {
	...
	public handleUserItemTypeSelect($event: Event): void {
		const selectedItemType: string = $event as unknown as string;
		if (selectedItemType === 'Техника') this.watchingChanged.emit({ vehicles: true, spares: false });
		if (selectedItemType === 'Запчасти') this.watchingChanged.emit({ vehicles: false, spares: true });
	}
