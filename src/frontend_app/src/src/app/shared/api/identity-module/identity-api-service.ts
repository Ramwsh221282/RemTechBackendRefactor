...
export class IdentityApiService {
	...
	resetPassword(login?: string | null | undefined, email?: string | null | undefined): Observable<Envelope> {
		return this.startResetingPassword(login, email);
	}

	confirmTicket(accountId: string, ticketId: string): Observable<Envelope> {
		const params: HttpParams = new HttpParams().set('accountId', accountId).set('ticketId', ticketId);
	...
	confirmPasswordReset(accountId: string, ticketId: string, newPassword: string): Observable<Envelope> {
	...
	private startLogout(): Observable<Envelope> {
		...
		this._logout$ = this._httpClient.post<Envelope>(requestUrl, null, { withCredentials: true }).pipe(
			finalize(() => {
	...
	private startRegisteringAccount(email: string, login: string, password: string): Observable<TypedEnvelope<any>> {
		if (this._registerAccount$) return this._registerAccount$;
		const requestUrl: string = `${this._url}/sign-up`;
		const payload: RegisterAccountRequest = CreateRegisterAccountRequest(email, login, password);
		this._registerAccount$ = this._httpClient.post<any>(requestUrl, payload).pipe(
			finalize((): void => {
	...
	private startChangingPassword(id: string, newPassword: string, currentPassword: string): Observable<Envelope> {
		...
		this._changePassword$ = this._httpClient.patch<Envelope>(requestUrl, payload, { withCredentials: true }).pipe(
			finalize(() => {
	...
	private startConfirmingPasswordReset(accountId: string, ticketId: string, newPassword: string): Observable<Envelope> {
		...
		const payload: CommitPasswordResetRequest = CreateCommitPasswordResetRequest(newPassword);
		this._confirmResetPassword$ = this._httpClient.post<Envelope>(requestUrl, payload).pipe(
			shareReplay(1),
	...
	private startRefreshingToken(): Observable<Envelope> {
		...
		this._refresh$ = this._httpClient.put<Envelope>(`${this._url}/refresh`, null, { withCredentials: true }).pipe(
	...
	private startResetingPassword(login?: string | null | undefined, email?: string | null | undefined): Observable<Envelope> {
		if (this._resetPassword$) return this._resetPassword$;
		const requestUrl: string = `${this._url}/reset-password`;
		const payload: ResetPasswordRequest = CreateResetPasswordRequest(login, email);
		this._resetPassword$ = this._httpClient.post<Envelope>(requestUrl, payload).pipe(
