...
export class LocationsApiService {
  ...
  private invokeLocationsFetch(
    ...
    if (query.Id) params = params.append('id', query.Id);
    if (query.CategoryId)
      params = params.append('category-id', query.CategoryId);
    if (query.BrandId) params = params.append('brand-id', query.BrandId);
    if (query.ModelId) params = params.append('model-id', query.ModelId);
    if (query.CategoryName)
      params = params.append('category-name', query.CategoryName);
    if (query.BrandName) params = params.append('brand-name', query.BrandName);
    if (query.ModelName) params = params.append('model-name', query.ModelName);
    ...
    this.fetchingLocations$ = this._httpClient
      .get<TypedEnvelope<LocationResponse[]>>(requestUrl, { params })
      .pipe(
        map(
          (envelope: TypedEnvelope<LocationResponse[]>): LocationResponse[] => {
