...
public sealed class ChangeMailerSmtpPasswordUseCaseTests(CompositionRootFixture services)
    : IClassFixture<CompositionRootFixture>
{
    ...
    private async Task Change_Mailer_Smtp_Password_Success()
    {
        ...
        IOptions<MailersEncryptOptions> encrypt =
            scope.ServiceProvider.GetRequiredService<IOptions<MailersEncryptOptions>>();
        CreateMailerUseCase createMailer = scope.ServiceProvider.GetRequiredService<CreateMailerUseCase>();
        ChangeMailerSmtpPasswordUseCase changePassword =
            scope.ServiceProvider.GetRequiredService<ChangeMailerSmtpPasswordUseCase>();
