...
/// <summary>
/// Расширения для регистрации заданий с расписанием Cron в контейнере служб.
/// </summary>
		...
		public void AddCronScheduledJobs()
		{
			...
			services.AddQuartz(q =>
			{
				foreach (Type jobType in jobTypes)
				{
					...
					CronScheduleAttribute? cronSchedule =
						jobType.GetCustomAttribute<CronScheduleAttribute>()
						?? throw new ApplicationException(
							$"{typeName} is implementing: {nameof(ICronScheduleJob)} but has no {nameof(CronScheduleAttribute)} on it."
						);
					...
					q.AddTrigger(t =>
						t.ForJob(jobKey).WithIdentity(triggerKey).WithCronSchedule(cronSchedule.Schedule)
					);
				}
			});
		}
