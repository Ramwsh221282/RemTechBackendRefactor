...
internal readonly ref struct EmbeddingData
{
    ...
    {
        using IDisposableReadOnlyCollection<DisposableNamedOnnxValue>? tokenizerResults = tokenizerSession.Run(tokenizerInputs);
        Tensor<int>? tokensTensor = tokenizerResults[0].AsTensor<int>();
        Tensor<int>? indicesTensor = tokenizerResults[2].AsTensor<int>();
        int[] tokens = tokensTensor.ToArray();
        int[] indices = indicesTensor.ToArray();
    ...
    private static void QuickSort(Span<int> idx, ReadOnlySpan<int> indices)
    {
        ...
        while (left <= right)
        {
            while (indices[idx[left]] < pivot) left++;
            while (indices[idx[right]] > pivot) right--;
