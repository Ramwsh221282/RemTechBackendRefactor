...
public sealed record ServiceCreatorInfo
{
    ...
    public static Result<ServiceCreatorInfo> Create(Guid creatorId, string type, string domain)
    {
        ...
        return domain.Length > MaxLength
            ? (Result<ServiceCreatorInfo>)Error.Validation($"Домен сохраняемого элемента не может превышать {MaxLength} символов.")
            : (Result<ServiceCreatorInfo>)new ServiceCreatorInfo(creatorId, type, domain);
    }
