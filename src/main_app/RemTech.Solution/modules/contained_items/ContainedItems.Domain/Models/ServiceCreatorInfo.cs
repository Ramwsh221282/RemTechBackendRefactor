    ...
    public static Result<ServiceCreatorInfo> Create(Guid creatorId, string type, string domain)
    {
        if (creatorId == Guid.Empty)
            return Error.Validation(
                "Идентификатор создателя сохраняемого элемента не может быть пустым."
            );
        ...
        if (type.Length > MaxLength)
            return Error.Validation(
                $"Тип сохраняемого элемента не может превышать {MaxLength} символов."
            );
        if (domain.Length > MaxLength)
            return Error.Validation(
                $"Домен сохраняемого элемента не может превышать {MaxLength} символов."
            );
