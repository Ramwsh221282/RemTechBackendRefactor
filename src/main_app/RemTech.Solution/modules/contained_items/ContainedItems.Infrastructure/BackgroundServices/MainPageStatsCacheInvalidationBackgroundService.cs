...
public sealed class MainPageStatsCacheInvalidationBackgroundService(
    ...
    ) : BackgroundService
{
    ...
    private async Task Execute(CancellationToken ct)
    {
        try
        {
            logger.Information("Invalidating main page stats cache.");
            await using AsyncServiceScope scope = serviceProvider.CreateAsyncScope();
            HybridCache cache = scope.ServiceProvider.GetRequiredService<HybridCache>();
            string key = BackgroundServicesUtils.CreateKey(new GetMainPageItemStatsQuery());
            await cache.RemoveAsync(key, ct);
            logger.Information("Main page stats cache invalidated.");
        }
        catch(Exception ex)
        {
