...
{
    public async Task<MainPageItemStatsResponse> Handle(
    ...
    private static async Task<MainPageItemStatsResponse> MapToResponse(
        DbDataReader reader,
    ...
    )
    {
        ...
        return new MainPageItemStatsResponse(
            [.. itemStats],
            [.. brandsPopularities],
            [.. categoriesPopularities]
        );
    }

    private static void ParseItemStatAndFillCollection(
        string itemStatJson,
        List<ItemStats> collection
    )
    {
    ...
    private static void ParseBrandStatAndFillCollection(
        string brandStatJson,
        List<BrandsPopularity> collection
    )
    {
    ...
    )
    {
        ...
        foreach (JsonElement element in jsonDoc.RootElement.EnumerateArray())
        {
            CategoriesPopularity? categoryStat = CategoryFromJson(
                element.GetProperty("categories_stat")
            );
