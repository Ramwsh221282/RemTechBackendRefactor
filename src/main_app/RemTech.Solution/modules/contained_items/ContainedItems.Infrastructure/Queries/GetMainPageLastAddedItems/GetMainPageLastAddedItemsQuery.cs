    ...
    : IQueryHandler<GetMainPageLastAddedItemsQuery, MainPageLastAddedItemsResponse>
{
    ...
    private static async Task<MainPageLastAddedItemsResponse> MapToResponse(
        DbDataReader reader,
    ...
    private static void ParseAndAddSpareItem(
        string spareDataJson,
        List<MainPageLastAddedItem> collection
    )
    {
    ...
    private static void ParseAndAddVehicleItem(
        string vehicleDataJson,
        List<MainPageLastAddedItem> collection
    )
    {
        ...
        if (string.IsNullOrWhiteSpace(title))
            return;
        string[]? photos = JsonSerializer.Deserialize<string[]>(
            doc.RootElement.GetProperty("photos")
        );
        photos ??= [];
        List<VehicleDataCharacteristic> characteristics = [];

        foreach (
            JsonElement element in doc.RootElement.GetProperty("characteristics").EnumerateArray()
        )
        {
            ...
            string characteristic =
                element.GetProperty("characteristic").GetString() ?? string.Empty;
