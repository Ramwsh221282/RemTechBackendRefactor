...
public sealed class Dev_ChangeEmailHandler(
...
) : ICommandHandler<Dev_ChangeEmailCommand, Unit>
{
    public async Task<Result<Unit>> Execute(
        Dev_ChangeEmailCommand command,
    ...
    )
    {
        ...
        Error error(Result<Account> account, Result<Unit> change) =>
        ...
        return (account, change) switch
        {
            ...
            { account.IsFailure: true, change.IsFailure: true } => error(account, change),
            _ => await saving(),
        };
    }
    ...
    private async Task<Result<Account>> ResolveAccount(
        ...
        command switch
        {
            ...
            { Email: not null } when !string.IsNullOrWhiteSpace(command.Email) =>
                await repository.Get(
                    new AccountSpecification().WithEmail(command.Email).WithLock(),
                    ct
                ),
            { Login: not null } when !string.IsNullOrWhiteSpace(command.Login) =>
                await repository.Get(
                    new AccountSpecification().WithLogin(command.Login).WithLock(),
                    ct
                ),
            _ => Error.Validation(
                "Не предоставлен ни Email, ни логин, ни Id аккаунта для изменения Email."
            ),
        };
}
