...
public sealed class Dev_ChangePasswordHandler(
    ...
    IAccountsModuleUnitOfWork unitOfWork) : ICommandHandler<Dev_ChangePasswordCommand, Unit>
{
    public async Task<Result<Unit>> Execute(Dev_ChangePasswordCommand command, CancellationToken ct = new CancellationToken())
    {
        ...
        if (change.IsFailure) return change.Error;
