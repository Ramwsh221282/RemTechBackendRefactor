    ...
    )
    {
        if (command.AccountId.HasValue)
            return await accounts.Get(
                new AccountSpecification().WithId(command.AccountId.Value),
                ct: ct
            );
        if (!string.IsNullOrWhiteSpace(command.AccountLogin))
            return await accounts.Get(
                new AccountSpecification().WithLogin(command.AccountLogin),
                ct: ct
            );
        if (!string.IsNullOrWhiteSpace(command.AccountEmail))
            return await accounts.Get(
                new AccountSpecification().WithEmail(command.AccountEmail),
                ct: ct
            );
