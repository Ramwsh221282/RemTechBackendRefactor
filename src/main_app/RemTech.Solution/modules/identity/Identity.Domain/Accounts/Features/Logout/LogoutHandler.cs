...
public sealed class LogoutHandler(IAccessTokensRepository accessTokens, IRefreshTokensRepository refreshTokens) : ICommandHandler<LogoutCommand, Unit>
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (accessToken.IsSuccess) await accessTokens.Remove(accessToken.Value, ct);
        if (refreshToken.IsSuccess) await refreshTokens.Delete(refreshToken.Value, ct);
        return Unit.Value;
    }
}