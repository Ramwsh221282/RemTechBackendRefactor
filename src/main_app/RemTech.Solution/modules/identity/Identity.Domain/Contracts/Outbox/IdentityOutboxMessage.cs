...
)
{
    ...
    public static IdentityOutboxMessage CreateNew<T>(string type, int retryCount, T payload) where T : IOutboxMessagePayload 
        => new(type, retryCount, JsonSerializer.Serialize(payload));
    
    public static IdentityOutboxMessage CreateNew<T>(string type, T payload) where T : IOutboxMessagePayload 
        => new(type, 0, JsonSerializer.Serialize(payload));
