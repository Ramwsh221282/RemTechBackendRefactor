...
public sealed class AddPermissionsHandler(IPermissionsRepository repository) : ICommandHandler<AddPermissionsCommand, IEnumerable<Permission>>
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (HasDuplicates(existing, out Error error)) return error;
    ...
    private bool HasDuplicates(IEnumerable<Permission> duplicates, out Error error)
    {
        ...
        if (duplicates.Any())
        {
            string message = $"Найдены дублирующиеся разрешения: {string.Join(" ", duplicates.Select(e => e.Name.Value))}";
    ...
    private IEnumerable<Permission> CreatePermissions(IEnumerable<AddPermissionCommandPayload> payloads) =>
        payloads.Select(p => Permission.CreateNew(PermissionName.Create(p.Name), PermissionDescription.Create(p.Description)));

    private IEnumerable<PermissionSpecification> CreateSpecificationsForExistanceCheck(
        IEnumerable<Permission> permissions) => permissions.Select(p => new PermissionSpecification().WithName(p.Name.Value));

    private async Task<IEnumerable<Permission>> GetExistingPermissions(
        IEnumerable<PermissionSpecification> specifications, 
        CancellationToken ct) => await repository.GetMany(specifications, ct);
}