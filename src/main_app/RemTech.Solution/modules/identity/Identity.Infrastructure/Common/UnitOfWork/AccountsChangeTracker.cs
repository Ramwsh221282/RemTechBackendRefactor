...
public sealed class AccountsChangeTracker(NpgSqlSession session)
{
    ...
    {
        foreach (Account account in accounts) Accounts.TryAdd(account.Id.Value, account.Copy());
    }
    ...
    {
        if (!added.Item2.Any()) return;
        var parameters = added.Item2.Select(p => new
        {
            account_id = added.Item1.Value,
    ...
    private (AccountId, IEnumerable<Permission> removed) GetRemovedPermissions(Account account, Account original) =>
        (account.Id, original.Permissions.Permissions
            .ExceptBy(account.Permissions.Permissions.Select(p => p.Id.Value), p => p.Id.Value));
    
    private (AccountId accountId, IEnumerable<Permission> added) GetAddedPermissions(Account account, Account original) =>
        (account.Id, account.Permissions.Permissions
            .ExceptBy(original.Permissions.Permissions.Select(p => p.Id.Value), p => p.Id.Value));
    ...
    private async Task SaveAccountChanges(IEnumerable<Account> tracking, CancellationToken ct)
    {
        ...
        {
            string clause = string.Join(" ", tracking.Select((a, i) =>
            {
