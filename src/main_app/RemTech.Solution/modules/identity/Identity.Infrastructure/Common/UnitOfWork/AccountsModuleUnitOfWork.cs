    ...
    : IAccountsModuleUnitOfWork
{
    ...
    public async Task Save(IEnumerable<IdentityOutboxMessage> messages, CancellationToken ct = default) => await OutboxMessages.Save(messages, ct);
    public async Task Save(IdentityOutboxMessage message, CancellationToken ct = default) => await OutboxMessages.Save([message], ct);
