...
public static class ChangePasswordPersisting
{
    ...
    extension(ChangePassword origin)
    {
        public ChangePassword WithPersisting(IServiceProvider sp)
        {
            ...
            return ChangePassword(origin, storage);
        }

        public ChangePassword WithTranscation(IServiceProvider sp)
        {
            NpgSqlSession session=  sp.Resolve<NpgSqlSession>();
            return ChangePassword(origin, session);
        }
    }
}