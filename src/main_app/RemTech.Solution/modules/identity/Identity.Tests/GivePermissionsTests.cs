...
public sealed class GivePermissionsTests(IntegrationalTestsFactory factory) : IClassFixture<IntegrationalTestsFactory>
{
    ...
    private async Task Give_Account_Permissions_Success()
    {
        ...
        string accountEmail = "testEmail@mail.com";
        string accountPassword = "simplePassword@123";
        Permission[] permissions = (await Services.GetPermissions()).ToArray();
        
        Result<Unit> account = await Services.InvokeAccountRegistration(new RegisterAccountCommand(accountEmail, accountName, accountPassword));
        Assert.True(account.IsSuccess);
        
        Result<Account> registered = await Services.GetAccountByEmail(accountEmail);
        Assert.True(registered.IsSuccess);
        
        Result<Account> withPermissions = await Services.GivePermissions(registered.Value.Id.Value, permissions.Select(p => p.Id.Value));
