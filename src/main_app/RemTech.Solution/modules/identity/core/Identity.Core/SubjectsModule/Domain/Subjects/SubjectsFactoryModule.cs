...
public static class SubjectsFactoryModule
{
    extension(Subject)
    {
        ...
        {
            return metadata.Continue(credentials).Continue(activation).Continue(permissions)
                           .Map(() => new Subject(metadata, credentials, activation, permissions, new SubjectTickets([])));
        }
        ...
        public static Result<Subject> Create(Result<SubjectMetadata> metadata, Result<SubjectCredentials> credentials)
        {
            ...
            return metadata.Continue(credentials).Map(() => new Subject(metadata, credentials, activation, permissions, new SubjectTickets([])));
        }
        ...
        public static Result<Subject> Create(SubjectSnapshot snapshot)
        {
            ...
            return Create(metadata, credentials, activation, collection);
        }
    }
}