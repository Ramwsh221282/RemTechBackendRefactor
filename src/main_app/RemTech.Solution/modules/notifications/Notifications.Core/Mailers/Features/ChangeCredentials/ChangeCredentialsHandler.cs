...
{
    public async Task<Result<Mailer>> Execute(ChangeCredentialsCommand command, CancellationToken ct = new CancellationToken())
    {
    ...
    private async Task<MailerCredentials> CreateEncryptedCredentials(ChangeCredentialsCommand command, CancellationToken ct) =>
