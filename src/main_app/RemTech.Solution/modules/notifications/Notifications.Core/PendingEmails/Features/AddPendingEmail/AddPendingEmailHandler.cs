...
public sealed class AddPendingEmailHandler(
    IPendingEmailNotificationsRepository repository)
    : ICommandHandler<AddPendingEmailCommand, Unit>
{
    public async Task<Result<Unit>> Execute(AddPendingEmailCommand command, CancellationToken ct = new CancellationToken())
    {
