...
public sealed class PendingEmailNotification(
    Guid id,
    string recipient,
    string subject,
    string body,
    ...
    )
{
    ...
    {
        List<string> errors = [];
        if (string.IsNullOrWhiteSpace(recipient)) errors.Add("Почта поле не может быть пустой");
        if (string.IsNullOrWhiteSpace(subject)) errors.Add("Тема письма не может быть пустой");
        if (string.IsNullOrWhiteSpace(body)) errors.Add("Текст письма не может быть пустым");
        if (errors.Count > 0)
        {
