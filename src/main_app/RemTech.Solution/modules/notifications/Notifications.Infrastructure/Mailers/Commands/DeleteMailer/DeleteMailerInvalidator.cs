...
{
    public async Task InvalidateCache(DeleteMailerCommand command, Guid result, CancellationToken ct = new CancellationToken())
    {
        Task[] tasks = 
        [
            arrayCacheInvalidator.Invalidate(ct),
            recordCacheInvalidator.Invalidate(result, ct)
        ];
