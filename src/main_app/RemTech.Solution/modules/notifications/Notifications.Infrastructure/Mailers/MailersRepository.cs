...
public sealed class MailersRepository(NpgSqlSession session, INotificationsModuleUnitOfWork changeTracker) : IMailersRepository
{
    ...
    public async Task<Result<Mailer>> Get(MailersSpecification specification, CancellationToken ct = default)
    {
        ...
        if (mailer is null) return Error.NotFound("Не найдена конфигурация почтового сервиса.");
