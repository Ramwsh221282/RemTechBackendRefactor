    ...
    IOptions<FrontendOptions> frontendOptions) : IConsumer
{
    ...
    private AsyncEventHandler<BasicDeliverEventArgs> Handler => async (_, @event) =>
    {
        ...
        try
        {
            NewAccountRegisteredOutboxMessagePayload payload = NewAccountRegisteredOutboxMessagePayload.FromDeliverEventArgs(@event);
        ...
        string Login)
    {
        ...
        {
            List<string> errors = [];
            if (AccountId == Guid.Empty) errors.Add("Идентификатор аккаунта пуст");
            if (TicketId == Guid.Empty) errors.Add("Идентификатор тикета пуст");
            if (string.IsNullOrWhiteSpace(Email)) errors.Add("Почта пуста");
            if (string.IsNullOrWhiteSpace(Login)) errors.Add("Логин пуст");
