...
public static class NotificationsModuleTestsExtensions
{
    ...
    {
        public async Task PublishOnNewAccountCreated(Guid accountId, Guid ticketId, string email, string login)
        {
            FakeAccountRegisteredPublisher publisher = sp.GetRequiredService<FakeAccountRegisteredPublisher>();
            await publisher.Publish(accountId, ticketId, email, login);
        }

        public async Task<PendingEmailNotification[]> GetPendingEmailNotifications()
        {
            await using AsyncServiceScope scope = sp.CreateAsyncScope();
            IPendingEmailNotificationsRepository repository = scope.ServiceProvider.GetRequiredService<IPendingEmailNotificationsRepository>();
            PendingEmailNotificationsSpecification specification = new PendingEmailNotificationsSpecification().OfNotSentOnly();
