...
public sealed class StructuringEngineTypeCharacteristic : IStructuringCharacteristic
{
    private readonly NotEmptyString _name;
    private readonly NotEmptyString _value;
    private readonly NotEmptyGuid _id;

    public StructuringEngineTypeCharacteristic(NotEmptyString name, NotEmptyString value, NotEmptyGuid id)
    {
        _name = name;
        _value = value;
        _id = id;
    ...
    {
        ctx = null;
        if (_name != "Тип двигателя") return false;
        CharacteristicMeasure measure = new("тип");
