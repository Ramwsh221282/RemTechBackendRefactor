...
public sealed class StructuringLoadingHeightCharacteristic(NotEmptyString name, NotEmptyString value, NotEmptyGuid id)
    : IStructuringCharacteristic
{
    public bool Structure([NotNullWhen(true)] out ValuedCharacteristic? ctx)
    {
        ctx = null;
        if (name != "Высота подъёма") return false;
