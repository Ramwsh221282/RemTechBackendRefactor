...
public sealed class PresentingVehicle(Vehicle origin) : Vehicle(origin)
{
    public VehiclePresentation Present()
    {
        ...
        VehiclePhotosPresentation photos = new(Photos.Read().Select(p => new VehiclePhotoPresentation(p)));
        VehicleCharacteristicsPresentation characteristic = new(
            Characteristics
                .Read()
                .Select(c =>
                    new VehicleCharacteristicPresentation(
                        Identity.Read(), 
                        c.WhatCharacteristic().Identity.ReadId(), 
                        c.WhatCharacteristic().Identity.ReadText(), 
                        c.WhatValue(), 
                        c.WhatCharacteristic().Measure())));
        return new VehiclePresentation(identity, kind, brand, model, region, price, photos, characteristic);
    }
}