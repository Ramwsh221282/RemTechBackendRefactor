...
public sealed class VehiclePriceQuerySpecification(double? priceTo, double? priceFrom, bool? isNds = null)
    : IQueryVehiclesSpecification
{
    public void ApplyTo(VehiclesSqlQuery query)
    {
        ...
        if (isNds.HasValue)
            query.AcceptFilter("v.is_nds = @is_nds", new NpgsqlParameter<bool>("@is_nds", isNds.Value));
    }
}