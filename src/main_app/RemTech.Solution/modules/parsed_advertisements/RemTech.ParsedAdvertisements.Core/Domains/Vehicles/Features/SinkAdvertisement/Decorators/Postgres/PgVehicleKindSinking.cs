    ...
    : ITransportAdvertisementSinking
{
    public async Task<Status> Sink(IVehicleJsonSink sink, CancellationToken ct = default)
    {
        ...
        VehicleKind saved = await new PgVarietVehicleKindStorage()
            .With(new PgVehicleKindsStorage(connection))
            .With(new PgDuplicateResolvingVehicleKindStorage(connection))
            .Read(kind, ct);
