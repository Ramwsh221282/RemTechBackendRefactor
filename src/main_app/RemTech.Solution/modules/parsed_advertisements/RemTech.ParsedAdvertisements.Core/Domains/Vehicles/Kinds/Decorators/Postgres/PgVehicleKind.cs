...
{
    public async Task<VehicleKind> SaveAsync(PgVehicleKindToStoreCommand command, CancellationToken ct = default)
    {
        int affected = await command.Execute(connection, ct);
        return affected != 1 ? throw new OperationException("Дубликат типа техники по названию")
            : this;
    }

    public async Task<VehicleKind> SaveAsync(PgVehicleKindFromStoreCommand command, CancellationToken ct = default)
    {
