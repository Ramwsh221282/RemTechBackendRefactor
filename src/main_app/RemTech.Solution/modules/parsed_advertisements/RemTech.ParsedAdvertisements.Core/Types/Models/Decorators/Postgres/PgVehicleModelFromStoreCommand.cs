...
namespace RemTech.ParsedAdvertisements.Core.Types.Models.Decorators.Postgres;
...
public sealed class PgVehicleModelFromStoreCommand(string name)
{
    ...
    public async Task<VehicleModel> Fetch(
    ...
    )
    {
        if (string.IsNullOrWhiteSpace(_sql))
            throw new OperationException(
                "Невозможно найти модель техники поиском. Название модели техники было пустым."
            );
        await using DbDataReader reader = await new AsyncDbReaderCommand(
            new AsyncPreparedCommand(
                new ParametrizingPgCommand(new PgCommand(connection, _sql)).With("@text", name)
            )
        ).AsyncReader(ct);
