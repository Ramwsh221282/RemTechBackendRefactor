...
internal sealed class PgTgrmGeoLocationsStorage(NpgsqlDataSource dataSource) : IGeoLocationsStorage
{
    public async Task<GeoLocation> Save(GeoLocation geoLocation)
    {
        string sql = string.Intern(
            """
            SELECT id, text, kind, similarity(@text, text) as sml FROM parsed_advertisements_module.geos
            WHERE similarity(@text, text) > 0.2
            ORDER BY sml DESC
            LIMIT 1;
            """
        );
        ...
        command.Parameters.Add(new NpgsqlParameter<string>("@text", parameter));
