...
namespace RemTech.Vehicles.Module.Types.Kinds.Decorators.Postgres;
...
public sealed class PgVehicleKindFromStoreCommand(string text)
{
    ...
    public async Task<VehicleKind> Fetch(
    ...
    )
    {
        if (string.IsNullOrWhiteSpace(text))
            throw new OperationException(
                "Невозможно получить тип техники поиском. Параметр названия был пустым."
            );
