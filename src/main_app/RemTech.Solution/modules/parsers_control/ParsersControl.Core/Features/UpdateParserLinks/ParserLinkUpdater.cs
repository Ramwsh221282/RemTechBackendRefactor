...
public sealed class ParserLinkUpdater
{
    ...
    public Result Update(SubscribedParserLink link)
    {
        ...
        if (editing.IsFailure) return Result.Failure(editing.Error);
        if (Activity.HasValue)
        {
            if(Activity.Value) link.Enable();
            else link.Disable();
        }
        return editing;
    }
    
    public static Result<ParserLinkUpdater> Create(
        Guid id, 
        bool? activity, 
        string? name, 
        string? url)
    {
        ...
        return new ParserLinkUpdater(idResult.Value, activity, name, url);
    }
}