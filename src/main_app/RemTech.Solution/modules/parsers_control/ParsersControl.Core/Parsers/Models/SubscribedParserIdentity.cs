...
public sealed record SubscribedParserIdentity
{
    ...
    {
        List<string> errors = [];
        if (string.IsNullOrWhiteSpace(domainName)) errors.Add(Error.NotSet("Идентификатор домена сервиса парсера"));
        if (domainName.Length > MaxDomainNameLength) errors.Add(Error.GreaterThan("Идентификатор домена сервиса парсера", MaxDomainNameLength));
        if (string.IsNullOrWhiteSpace(serviceType)) errors.Add(Error.NotSet("Тип обрабатываемых данных парсером"));
        if (serviceType.Length > MaxServiceTypeLength) errors.Add(Error.GreaterThan("Тип обрабатываемых данных парсером", MaxServiceTypeLength));
