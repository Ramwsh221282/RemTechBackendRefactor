...
public sealed record SubscribedParserIdentity
{
    ...
    public static Result<SubscribedParserIdentity> Create(string domainName, string serviceType)
    {
        ...
        return errors.Count > 0 ? (Result<SubscribedParserIdentity>)Error.Validation(errors) : (Result<SubscribedParserIdentity>)new SubscribedParserIdentity(domainName, serviceType);
    }
}
