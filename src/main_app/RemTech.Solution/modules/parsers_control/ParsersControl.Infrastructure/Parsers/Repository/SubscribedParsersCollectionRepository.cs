...
public sealed class SubscribedParsersCollectionRepository(
    NpgSqlSession session,
    Serilog.ILogger logger) : ISubscribedParsersCollectionRepository
{
    private readonly SubscribedParsersCollectionChangeTracker _changeTracker = new(logger);
    
    public async Task<SubscribedParsersCollection> Get(SubscribedParsersCollectionQuery query, CancellationToken ct = default)
    {
        ...
        if (query.WithLock) await BlockParsers(collection.GetIdentifiers(), ct);
    ...
    private async Task<SubscribedParsersCollection> MapFromReader(DbDataReader reader, CancellationToken ct)
    {
    ...
    private static SubscribedParserLink MapParserLink(Guid id, SubscribedParser parser, IDataReader reader)
    {
        ...
        return SubscribedParserLink.Create(
            ...
            statistics: new ParsingStatistics(workTime: ParsingWorkTime.FromTotalElapsedSeconds(elapsedSeconds), parsedCount: ParsedCount.Create(processed)),
            active: active
            );
    }
    ...
    private (DynamicParameters parameters, string filterSql) FilterClause(SubscribedParsersCollectionQuery query)
    {
    ...
    private sealed class SubscribedParsersCollectionChangeTracker(Serilog.ILogger logger)
    {
        ...
        public async Task<Result<Unit>> SaveChanges(SubscribedParsersCollection parsers, NpgSqlSession session, CancellationToken ct = default)
        {
            int index = 0;
            DynamicParameters parameters = new();
            List<string> ids = [];
            ...
            foreach (SubscribedParser parser in parsers.Read())
            {
                ...
                if (parser.Schedule.WaitDays != existing.Schedule.WaitDays)
                {
                    ...
                    parameters.Add(waitDaysParam, parser.Schedule.WaitDays is null ? DBNull.Value : parser.Schedule.WaitDays.Value, DbType.Int32);
                ...
                if (parser.Schedule.NextRun != existing.Schedule.NextRun)
                {
                    ...
                    parameters.Add(nextRunParam, parser.Schedule.NextRun is null ? DBNull.Value : parser.Schedule.NextRun.Value, DbType.DateTime);
                ...
                if (parser.Schedule.FinishedAt != existing.Schedule.FinishedAt)
                {
                    ...
                    parameters.Add(finishedAtParam, parser.Schedule.FinishedAt is null ? DBNull.Value : parser.Schedule.FinishedAt.Value, DbType.DateTime);
                ...
                if (parser.Schedule.StartedAt != existing.Schedule.StartedAt)
                {
                    ...
                    parameters.Add(startedAtParam, parser.Schedule.StartedAt is null ? DBNull.Value : parser.Schedule.StartedAt.Value, DbType.DateTime);
            ...
            if (ids.Count == 0) return Result.Success(Unit.Value);
            if (stateCases.Count > 0) setClauses.Add($"state = CASE {string.Join(" ", stateCases)} ELSE state END");
            if (waitDaysCases.Count > 0) setClauses.Add($"wait_days = CASE {string.Join(" ", waitDaysCases)} ELSE wait_days END");
            if (nextRunCases.Count > 0) setClauses.Add($"next_run = CASE {string.Join(" ", nextRunCases)} ELSE next_run END");
            if (finishedAtCases.Count > 0) setClauses.Add($"finished_at = CASE {string.Join(" ", finishedAtCases)} ELSE finished_at END");
            if (startedAtCases.Count > 0) setClauses.Add($"started_at = CASE {string.Join(" ", startedAtCases)} ELSE started_at END");
            if (processedCases.Count > 0) setClauses.Add($"processed = CASE {string.Join(" ", processedCases)} ELSE processed END");
            if (elapsedSecondsCases.Count > 0) setClauses.Add($"elapsed_seconds = CASE {string.Join(" ", elapsedSecondsCases)} ELSE elapsed_seconds END");
            if (setClauses.Count == 0) return Result.Success(Unit.Value);
