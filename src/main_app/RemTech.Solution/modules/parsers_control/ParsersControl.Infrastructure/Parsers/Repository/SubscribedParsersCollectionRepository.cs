...
) : ISubscribedParsersCollectionRepository
{
    ...
    private async Task<SubscribedParsersCollection> MapFromReader(
        DbDataReader reader,
    ...
    private static SubscribedParserLink MapParserLink(
        Guid id,
    ...
    private (DynamicParameters parameters, string filterSql) FilterClause(
        SubscribedParsersCollectionQuery query
    )
    {
    ...
    private sealed class SubscribedParsersCollectionChangeTracker(Serilog.ILogger logger)
    {
        ...
        private readonly Serilog.ILogger _logger =
            logger.ForContext<SubscribedParsersCollectionChangeTracker>();
        ...
        )
        {
            ...
            foreach (SubscribedParser parser in parsers.Read())
            {
                ...
                if (parser.Schedule.WaitDays != existing.Schedule.WaitDays)
                {
                    ...
                    parameters.Add(
                        waitDaysParam,
                        parser.Schedule.WaitDays is null
                            ? DBNull.Value
                ...
                if (parser.Schedule.NextRun != existing.Schedule.NextRun)
                {
                    ...
                    parameters.Add(
                        nextRunParam,
                        parser.Schedule.NextRun is null
                            ? DBNull.Value
                ...
                if (parser.Schedule.FinishedAt != existing.Schedule.FinishedAt)
                {
                    ...
                    parameters.Add(
                        finishedAtParam,
                        parser.Schedule.FinishedAt is null
                            ? DBNull.Value
                ...
                if (parser.Schedule.StartedAt != existing.Schedule.StartedAt)
                {
                    ...
                    parameters.Add(
                        startedAtParam,
                        parser.Schedule.StartedAt is null
                            ? DBNull.Value
                ...
                if (
                    parser.Statistics.WorkTime.TotalElapsedSeconds
                    != existing.Statistics.WorkTime.TotalElapsedSeconds
                )
                {
            ...
            if (waitDaysCases.Count > 0)
                setClauses.Add(
                    $"wait_days = CASE {string.Join(" ", waitDaysCases)} ELSE wait_days END"
                );
            if (nextRunCases.Count > 0)
                setClauses.Add(
                    $"next_run = CASE {string.Join(" ", nextRunCases)} ELSE next_run END"
                );
            if (finishedAtCases.Count > 0)
                setClauses.Add(
                    $"finished_at = CASE {string.Join(" ", finishedAtCases)} ELSE finished_at END"
                );
            if (startedAtCases.Count > 0)
                setClauses.Add(
                    $"started_at = CASE {string.Join(" ", startedAtCases)} ELSE started_at END"
                );
            if (processedCases.Count > 0)
                setClauses.Add(
                    $"processed = CASE {string.Join(" ", processedCases)} ELSE processed END"
                );
