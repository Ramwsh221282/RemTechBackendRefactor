...
public sealed class UpdateWaitDaysGateway : IGateway<UpdateWaitDaysRequest, ParserScheduleUpdateResponse>
{
    private readonly IParserScheduleStorage _storage;
    private readonly ParserScheduleUpdatedListenerPipeline _pipeLine;
    
    public async Task<Result<ParserScheduleUpdateResponse>> Execute(UpdateWaitDaysRequest request)
    {
        ...
        Result<ParserSchedule> update = await observed.Update(waitDays: request.WaitDays, ct: ct);
        return update.IsFailure ? update.Error : new ParserScheduleUpdateResponse(update.Value);
    }

    public UpdateWaitDaysGateway(
