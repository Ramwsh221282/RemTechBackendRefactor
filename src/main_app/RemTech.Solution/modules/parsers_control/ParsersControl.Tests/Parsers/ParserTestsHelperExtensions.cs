...
/// <summary>
///   Вспомогательные расширения для тестов парсеров.
/// </summary>
public static class ParserTestsHelperExtensions
{
	extension(IServiceProvider services)
	{
		...
		public async Task<Result<IEnumerable<SubscribedParserLink>>> AddLink(
			Guid parserId,
			string linkUrl,
			string linkName
		)
		...
		{
			await using AsyncServiceScope scope = services.CreateAsyncScope();
			UpdateParserLinksCommand command = new(
				parserId,
				[new UpdateParserLinksCommandInfo(linkId, Activity: true)]
			);
			return await scope
				.ServiceProvider.GetRequiredService<
		...
		{
			await using AsyncServiceScope scope = services.CreateAsyncScope();
			ICommandHandler<EnableParserCommand, SubscribedParser> handler = scope.ServiceProvider.GetRequiredService<
		...
		{
			await using AsyncServiceScope scope = services.CreateAsyncScope();
			ICommandHandler<StartParserCommand, SubscribedParser> handler = scope.ServiceProvider.GetRequiredService<
		...
		public async Task<Result<SubscribedParserLink>> ChangeLinkActivity(Guid parserId, Guid linkId, bool activity)
		{
			await using AsyncServiceScope scope = services.CreateAsyncScope();
		...
		{
			await using AsyncServiceScope scope = services.CreateAsyncScope();
			ICommandHandler<StopParserWorkCommand, SubscribedParser> handler = scope.ServiceProvider.GetRequiredService<
		...
		public async Task<Result<SubscribedParserLink>> SetLinkParsedAmount(Guid parserId, Guid linkId, int amount)
		{
			await using AsyncServiceScope scope = services.CreateAsyncScope();
