...
public sealed class GetSparesQueryHandler(
    ...
    IOptions<GetSparesThresholdConstants> textSearchConstants) : IQueryHandler<GetSparesQuery, GetSparesQueryResponse>
{
    ...
    private async Task<GetSparesQueryResponse> CreateResponse(DbDataReader reader, CancellationToken ct)
    {
        ...
        while (await reader.ReadAsync(ct))
        {
            ...
            response.Spares.Add(new SpareResponse()
            {
    ...
    private void ApplyFilters(GetSparesQuery query, DynamicParameters parameters, List<string> filters)
    {
        ...
        if (!string.IsNullOrWhiteSpace(query.TextSearch)) ApplyTextSearch(query.TextSearch, parameters, filters);
        if (!string.IsNullOrWhiteSpace(query.Oem)) ApplyOemSearch(query.Oem, parameters, filters);
    }
