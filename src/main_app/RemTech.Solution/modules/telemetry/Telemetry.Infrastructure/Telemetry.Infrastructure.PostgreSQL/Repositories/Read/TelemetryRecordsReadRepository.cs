...
public sealed class TelemetryRecordsReadRepository(
    TelemetryServiceDbContext context,
    IEmbeddingGenerator generator
) : ITelemetryRecordsReadRepository
{
    public async Task<(IEnumerable<TelemetryRecord> Records, long Count)> Read(
        int page = 1,
        int pageSize = 20,
    ...
    )
    {
        ...
        List<TelemetryRecord> records = results
            .Select(r => new TelemetryRecord(
                TelemetryRecordId.Create(r.id),
                TelemetryInvokerId.Create(r.invoker_id),
                new TelemetryActionDetails(
                    TelemetryActionName.Create(r.name),
                    Enumerable.Empty<TelemetryActionComment>()
                ),
                TelemetryActionStatus.Create(r.status),
                TelemetryRecordDate.Create(r.occured_at)
            ))
            .ToList();
