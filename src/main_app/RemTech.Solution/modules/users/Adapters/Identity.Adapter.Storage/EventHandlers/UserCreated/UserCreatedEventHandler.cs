...
namespace Identity.Adapter.Storage.EventHandlers.UserCreated;

public sealed class UserCreatedEventHandler(Serilog.ILogger logger, IdentityDbContext context)
    : IDomainEventHandler<Domain.Users.Events.UserCreated>
{
    private const string Context = nameof(Domain.Users.Events.UserCreated);

    public async Task<Status> Handle(
        Domain.Users.Events.UserCreated @event,
