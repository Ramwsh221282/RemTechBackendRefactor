...
public sealed class NpgSqlCharacteristicsPersister(NpgSqlSession session, EmbeddingsProvider embeddings) : ICharacteristicsPersister
{
    ...
    public async Task<Result<Characteristic>> Save(Characteristic characteristic, CancellationToken ct = default)
    {
        ...
        if (HasFromExactSearch(result)) return MapToCharacteristicFromExactSearch(result);
        if (HasFromEmbeddingSearch(result)) return MapToCharacteristicFromEmbeddingSearch(result);
        await SaveAsNewCharacteristic(characteristic, vector, ct);
        return characteristic;
    }
    
    private DynamicParameters BuildParameters(Characteristic characteristic, Vector vector)
    {
