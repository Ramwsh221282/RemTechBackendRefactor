...
) : IQueryHandler<GetLocationsQuery, IEnumerable<LocationsResponse>>
{
    ...
    private static async Task<IEnumerable<LocationsResponse>> MapFromReader(
        DbDataReader reader,
    ...
    private static string AddIncludes(
        GetLocationsQuery query,
        Func<GetLocationsQuery, string>[] includeSource
    )
    {
        string[] includes =
        [
    ...
    private static string AddOrderBy(
        GetLocationsQuery query,
        Func<GetLocationsQuery, string>[] orderBySources
    )
    {
        string[] orderings =
        [
            .. orderBySources
                .Select(s => s.Invoke(query))
        ...
        return orderings.Length == 0
            ...
            : "ORDER BY "
                + string.Join(
                    ", ",
                    orderBySources
                        .Select(source => source.Invoke(query))
                        .Where(s => !string.IsNullOrWhiteSpace(s))
                );
    }
    ...
    private static string OrderByTextSearchSimilarity(GetLocationsQuery query)
    {
        ...
        {
            return !string.IsNullOrWhiteSpace(query.TextSearch)
                ? "TextSearchScore ASC"
                : string.Empty;
        }
