...
public sealed class NpgSqlVehiclesPersister(NpgSqlSession session, EmbeddingsProvider embeddings) : IVehiclesPersister
{
    public async Task<Result<Unit>> Persist(VehiclePersistInfo info, CancellationToken ct = default)
    {
        ...
        if (affected == 0) return Error.Conflict("Unable to save vehicle.");
    ...
    private static DynamicParameters CreateParameters(VehiclePersistInfo info, EmbeddingsProvider embeddings)
    {
        ...
        string jsonCharacteristics = JsonSerializer.Serialize(vehicle.Characteristics.Select(c => new { Name = c.Name.Value, c.Value.Value }));
    ...
    {
        string text = $"{vehicle.Text.Value.Trim()} {vehicle.Characteristics.Select(c => $"{c.Name.Value.Trim()} {c.Value.Value.Trim()}").Aggregate((a, b) => $"{a} {b}").Trim()} {location.Name.Value.Trim()}";
        return text;
    }

    private static async Task PersistCharacteristics(NpgSqlSession session, NpgsqlConnection connection, Vehicle vehicle, CancellationToken ct)
    {
        ...
        string insertClause = $"INSERT INTO vehicles_module.vehicle_characteristics (vehicle_id, characteristic_id, value, name) VALUES {string.Join(",", insertClauses)} ON CONFLICT DO NOTHING;";
        CommandDefinition command = new(insertClause, parameters, cancellationToken: ct, transaction: session.Transaction);
