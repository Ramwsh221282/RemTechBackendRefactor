    ...
    : IQueryHandler<GetVehiclesQuery, GetVehiclesQueryResponse>
{
    ...
    private static async Task<GetVehiclesQueryResponse> FormResponseUsingReader(DbDataReader reader)
    {
        ...
        while (await reader.ReadAsync())
        {
            ...
            response.Vehicles.Add(new VehicleResponse()
            {
                ...
                Characteristics = JsonSerializer.Deserialize<VehicleCharacteristicsResponse[]>(reader.GetString(reader.GetOrdinal("characteristics")))!
            });
        }
    ...
    {
        if (string.IsNullOrWhiteSpace(queryParameters.TextSearch)) return;
        const string sql = """
                           v.id IN (
        ...
        DynamicParameters parameters)
    {
        ...
        if (queryParameters.Sort == "NONE") return (parameters, string.Empty);
        if (queryParameters.SortFields is null) return (parameters, string.Empty);
        if (queryParameters.SortFields.Any() == false) return (parameters, string.Empty);
        ...
        if (sortMode is null) return (parameters, string.Empty);
        ...
        foreach (string field in queryParameters.SortFields)
        {
            ...
            if (orderByClause is not null) orderBySql.Add(orderByClause);
        }
        
        return orderBySql.Count == 0 ? (parameters, string.Empty) : (parameters, " ORDER BY " + string.Join(", ", orderBySql));
        ...
        GetVehiclesThresholdConstants searchOptions)
    {
        ...
        {
            filters.Add(queryParameters.IsNds.Value switch
            {
