...
{
    private readonly string _containerSelector = string.Intern("div[data-marker='item-view/item-price-container']");
    ...
    public async Task<ParsedVehiclePrice> Read()
    {
        ...
        IElementHandle priceElement = await new ValidSingleElementSource(
            new ParentElementSource(priceContainer)).Read(_priceValueSelector);
