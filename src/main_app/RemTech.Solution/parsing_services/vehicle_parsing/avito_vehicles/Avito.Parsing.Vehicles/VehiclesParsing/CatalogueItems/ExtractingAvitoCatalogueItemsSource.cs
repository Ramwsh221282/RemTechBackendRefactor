    ...
    : IAvitoCatalogueItemsSource
{
    ...
    public async Task<CatalogueItemsList> Read()
    {
        ...
        IElementHandle container = await new ValidSingleElementSource(new PageElementSource(page))
    ...
    {
        IElementHandle title = await new ValidSingleElementSource(new ParentElementSource(item))
            .Read(string.Intern(".iva-item-listTopBlock-n6Rva"));
        IElementHandle url = await new ValidSingleElementSource(new ParentElementSource(title))
