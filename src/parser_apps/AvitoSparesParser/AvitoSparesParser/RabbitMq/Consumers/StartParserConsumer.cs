...
) : IConsumer
{
    ...
    {
        Logger.Information(
            "Starting channel initialization {Exchange} {Queue} {RoutingKey}",
            Exchange,
            Queue,
            RoutingKey
        );
        ...
        Logger.Information(
            "Channel {Exchange} {Queue} {RoutingKey} initialized.",
            Exchange,
            Queue,
            RoutingKey
        );
        ...
        async (_, @event) =>
        {
            ...
            try
            {
                ...
                ProcessingParserLink[] links =
                    IEnumerable<ProcessingParserLink>.ArrayFromDeliverEventArgs(@event);
                if (links.Length == 0)
                {
                    Logger.Information(
                        "No links for parser {Domain} {Type}",
                        parser.Domain,
                        parser.Type
                    );
                    await Channel.BasicAckAsync(@event.DeliveryTag, false);
                ...
                if (commit.IsFailure)
                {
                    Logger.Error(
                        commit.Error,
                        "Failed to commit transaction for parser {Domain} {Type}",
                        parser.Domain,
                        parser.Type
                    );
                    await Channel.BasicAckAsync(@event.DeliveryTag, false);
                ...
                Logger.Information(
                    "Parser {Domain} {Type} has been registered with links count: {Count}.",
                    parser.Domain,
                    parser.Type,
                    links.Length
                );
