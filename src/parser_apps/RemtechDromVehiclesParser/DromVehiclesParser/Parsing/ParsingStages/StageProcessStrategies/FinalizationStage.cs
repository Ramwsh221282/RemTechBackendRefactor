...
public static class FinalizationStage
{
    ...
    private static AddContainedItemsMessage CreateAddContainedItemsMessage(
        WorkingParser parser,
        DromAdvertisementFromPage[] advertisements
    )
    {
        return new AddContainedItemsMessage()
        {
            ...
            Items = [.. advertisements.Select(CreateAddContainedItemsMessagePayload)],
            ItemType = "Техника",
        };
    }

    private static AddContainedItemsMessagePayload CreateAddContainedItemsMessagePayload(
        DromAdvertisementFromPage advertisement
    )
    {
        ...
        string content = JsonSerializer.Serialize(payload, _options);
        return new AddContainedItemsMessagePayload() { ItemId = id, Content = content };
    }

    private static async Task FinalizeParsing(
        NpgSqlSession session,
    ...
    private static async Task FinishParser(
        NpgSqlSession session,
    ...
    private static async Task<DromAdvertisementFromPage[]> GetAdvertisementsForFinalization(
        NpgSqlSession session
    )
    {
    ...
    private static async Task RemoveWorkingParserLinksInformation(
        NpgSqlSession session,
        CancellationToken ct
    )
    {
    ...
    private static async Task RemoveCataloguePagesInformation(
        NpgSqlSession session,
        CancellationToken ct
    )
    {
