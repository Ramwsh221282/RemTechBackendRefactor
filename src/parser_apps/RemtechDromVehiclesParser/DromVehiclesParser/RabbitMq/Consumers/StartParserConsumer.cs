...
) : IConsumer
{
    ...
    {
        Logger.Information(
            "Initializing channel {Exchange} {Queue} {RoutingKey}",
            Exchange,
            Queue,
            RoutingKey
        );
    ...
    {
        Logger.Information(
            "Shutting down channel {Exchange} {Queue} {RoutingKey}",
            Exchange,
            Queue,
            RoutingKey
        );
        ...
        Logger.Information(
            "Channel {Exchange} {Queue} {RoutingKey} shut down.",
            Exchange,
            Queue,
            RoutingKey
        );
    ...
    private AsyncEventHandler<BasicDeliverEventArgs> Handler =>
        async (_, @event) =>
        {
            ...
            try
            {
                ...
                ParserWorkStage stage = ParserWorkStage
                    .FromDeliverEventArgs(@event)
                    .PaginationStage();
